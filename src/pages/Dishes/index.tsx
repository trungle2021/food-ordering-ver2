...
export const DishPage = () => {
  ...
  const [checkedCategories, setCheckedCategories] =
    useState<CheckedCategoriesProps>({});
  ...
  const handleCategoryCheckBoxChange = (
    dataChanged: CheckedCategoriesProps,
    categoryIdChanged: string
  ) => {
    const isChecked = dataChanged[categoryIdChanged];
    const category = categories.find(
      (category) => category._id === categoryIdChanged
    );
    ...
    if (isChecked) {
      ...
      const updatedCategories = queryParams
      ...
      updatedCategories.forEach((name) =>
  ...
  const handleClickSortBy = (filter: { sort: string }) => () => {
    ...
    if (
      sortByParams.includes("Price-Ascending") &&
      sort === "Price-Descending"
    ) {
      newSortByParams.delete("Price-Ascending");
    } else if (
      sortByParams.includes("Price-Descending") &&
    ...
    Array(...newSortByParams).forEach((param) =>
  ...
  const handleClearFilter =
    (key: string, value: string) => (event: SyntheticEvent) => {
      ...
      switch (key) {
        ...
        case "sort":
          const updatedSortBy = queryParams
            .getAll("sort")
            .filter((filter) => filter !== value);
          queryParams.delete("sort");
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          ...
          <div style={{ textAlign: "center", position: "relative" }}>
            {(applyingFilter.category_name ||
              applyingFilter.sort ||
              applyingFilter.price) && (
              <div style={{ padding: "10px 0" }}>
                ...
                >
                  {Object.entries(applyingFilter).map(
            ...
            <Accordion disableGutters>
              ...
              <AccordionDetails id="sortBy-content">
                <FormGroup>
                  ...
                  >
                    <Button
                      onClick={handleClickSortBy({ sort: "Price-Ascending" })}
                    >
                      Price (lowest - highest)
                    </Button>
                    <Button onClick={handleClickSortBy({ sort: "Newest" })}>
                      Newest
                    </Button>
                    <Button
                      onClick={handleClickSortBy({ sort: "Price-Descending" })}
                    >
                ...
                return (
                  <Grid item key={dish._id} xs={12} sm={6} md={3} lg={3}>
                    <DishCard
                      ...
                      name={dish.name}
                      price={dish.price}
                      favorite_info={dish.favorite_info ?? undefined}
                    />
                  </Grid>
                );
              })
